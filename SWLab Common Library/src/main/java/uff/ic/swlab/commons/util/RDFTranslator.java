...
public class RDFTranslator {
    ...
    public static void write(OutputStream output, Model model, Lang lang) {
        if (lang == RDFLanguages.nameToLang("RDFa"))
            writeHTML(output, model, redirectOpenLinkURIBurner(output, model));
        else
            writeHTML(output, model, null);
    }

    public static String redirectOpenLinkURIBurner(OutputStream output, Model model) {
        StringWriter writer = new StringWriter();
        model.write(writer, Lang.RDFXML.getName());

        //String uri = "http://rdf-translator.appspot.com/convert/detect/rdfa/content";
        String uri = "http://linkeddata.uriburner.com/about/html/";
        ...
        try {
            ...
            Logger.getLogger(RDFTranslator.class.getName()).log(Level.SEVERE, null, e);
        } catch (IOException e) {
            Logger.getLogger(RDFTranslator.class.getName()).log(Level.SEVERE, null, e);
        }
        return "";
    ...
    private static void writeHTML(OutputStream output, Model model, String rdfa) {
        String response = "";
