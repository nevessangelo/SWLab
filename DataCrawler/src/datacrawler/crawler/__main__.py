...
def _initialize(recreateDB=False):
    ...
    with MongoClient() as client:
        ...
        if (recreateDB): MongoClient().drop_database(db_name); print('DB has been recreated.')
        for c in DB.collection_names():
            if (c not in ['catalogs', 'deletions', 'datasets', 'voids', 'system.indexes']): DB.drop_collection(c)
